...
export default function ReportSidebar({ scenario, hypothesis = "", onHypothesisChange, onSubmitHypothesis }: Props) {
  return (
    <div className="report">
      ...
      <div className="meta-row"><span className="k">Endpoint</span><span className="v mono" style={{ fontSize: 11 }}>{scenario.report.metadata.endpoint}</span></div>
      <div className="meta-row"><span className="k">Error rate</span><span className="v" style={{ color: "var(--coral)" }}>{scenario.report.metadata.errorRate}</span></div>
      <div className="meta-row"><span className="k">First seen</span><span className="v">{scenario.report.metadata.firstSeen}</span></div>
