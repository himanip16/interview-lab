...
const CARDS = [
  ...
  },
  {
    ...
    href: "/interview/setup?type=pr_review",
    icon: (
      <svg viewBox="0 0 84 84" fill="none">
        <circle cx="42" cy="42" r="40" fill="#fff"/>
          ...
          >
            {CARDS.map((card) => (
              <div
                ...
                style={{ background: `linear-gradient(160deg, ${card.gradient.split(" ")[0].replace("from-[", "").replace("]", "")}, ${card.gradient.split(" ")[1].replace("to-[", "").replace("]", "")})` }}
              >
